...
async function handleUnionOrderDeepLink() {
  const unionOrderId = route.query.unionOrderId;
  if (!unionOrderId || typeof unionOrderId !== 'string') return;
  query.mchOrderNo = unionOrderId;
...
onMounted(async () => {
  ...
  if (route.query.unionOrderId) {
    await handleUnionOrderDeepLink();
  } else {
    applyToday();
    await load();
  }
