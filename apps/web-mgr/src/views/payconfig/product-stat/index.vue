...
defineOptions({ name: 'ProductStatPage' });
...
const columns: TableColumnsType = [
  ...
  { dataIndex: 'productName', title: '产品类型', width: 140 },
  { dataIndex: 'totalSuccessAmount', title: '成交金额', width: 120 },
  { dataIndex: 'totalAmount', title: '订单总额', width: 120 },
...
async function loadStat() {
  try {
    stat.value = (await fetchProductStatCountApi(buildParams())) ?? {};
  } catch {
    // ignore
  }
}
...
async function loadData(resetPage = false) {
  ...
  try {
    void loadStat();
    const page = await fetchProductStatApi(buildParams());
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.dataIndex === 'totalAmount'">
            <b>{{ formatYuan(record.totalAmount as number) }}</b>
          </template>
